...
class RunSpec(StoreSerializable):
    ...
    miscibility: MiscibilityModel = "immiscible"
    ...
    **Properties affected when Pb is frozen:**
    ...
    pressure_solver: SolverStr | Iterable[SolverStr] = "bicgstab"
    """Pressure system solver(s) (can be a list of solver to use in sequence)."""

    transport_solver: SolverStr | Iterable[SolverStr] = "bicgstab"
    ...
    **Guidelines by reservoir pressure:**
    ...
    def convert(
        ...
        *,
        ...
        """
        Converts the run spec to a different unit system.

        :param target: Target unit system.
        :param table: Optional conversion table. If not provided, a default table will be used.
        :returns: A new `RunSpec` instance with properties converted to the target unit system.
        """
        ...
        return attrs.evolve(
            self,
            control_spec=self.control_spec.convert(target, table=table),
            maximum_pressure_change=scale(self.maximum_pressure_change, pressure_factor),
            unit_system=target,
        )
